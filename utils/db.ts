...
export async function getFoldersByParentId(
  ...
  const iter = kv.list<Folder>({
    prefix: ["folders_by_parent_folder", parentId],
  });
  const folders = [];
...
export async function getFilesByParentId(
  ...
  const iter = kv.list<FreshFile>({
    prefix: ["files_by_parent_folder", parentId],
  });
  const files = [];
  for await (const { value } of iter) {
    files.push(value);
